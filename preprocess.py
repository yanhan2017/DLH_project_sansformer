...
#%%
admits = pd.read_csv(
    os.path.join(CORE_PATH, "admissions.csv.gz"),
    parse_dates=["admittime", "dischtime"],
    compression="gzip",
)
patients = pd.read_csv(os.path.join(CORE_PATH, "patients.csv.gz"), compression="gzip")
...
diags = pd.read_csv(os.path.join(HOSP_PATH, "diagnoses_icd.csv.gz"), compression="gzip")
...
grouped_diag.to_feather(os.path.join(MIMIC_PATH, "grouped_diag.feather"))

# grouped_diag = pd.read_feather(MIMIC_PATH/"grouped_diag.feather")

# processing procedure codes
procs = pd.read_csv(os.path.join(HOSP_PATH, "procedures_icd.csv.gz"), compression="gzip")
...
grouped_proc.to_feather(os.path.join(MIMIC_PATH, "grouped_proc.feather"))

# process drg codes
drgs = pd.read_csv(os.path.join(HOSP_PATH, "drgcodes.csv.gz"), compression="gzip")
drgs.drop(
...
grouped_drg.to_feather(os.path.join(MIMIC_PATH, "grouped_drg.feather"))

# process services
services = pd.read_csv(os.path.join(HOSP_PATH, "services.csv.gz"), compression="gzip")
...
grouped_service.to_feather(os.path.join(MIMIC_PATH, "grouped_service.feather"))
...
grouped_patient.to_feather(os.path.join(MIMIC_PATH, f"grouped_patient_rem{TARGET_OFFSET}.feather"))

#%%
mimic_df = pd.read_feather(os.path.join(MIMIC_PATH, f"grouped_patient_rem{TARGET_OFFSET}.feather"))
# %%
...
all_ethn = []
for idx in mimic_df.index:
    all_ethn += mimic_df.loc[idx, "race"].split(";")
...
mimic_df["encoded_ethnicity"] = mimic_df["race"].apply(
...
train.reset_index(drop=True).to_feather(
    os.path.join(MIMIC_PATH, f"mimic_train_rem{TARGET_OFFSET}.feather")
)
